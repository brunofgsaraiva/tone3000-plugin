...
test('pickModel takes the exact name, or the only model, and never guesses', () => {
  ...
  assert.equal(pickModel([{ id: 1, name: `  ${IDENTITY.name}  ` }] as never, IDENTITY)?.id, 1);
...
test('stashAddressOf reads the content address out of a file:// stash URL', () => {
  assert.deepEqual(stashAddressOf('file:///stash/TONE3000/LocalModels/1a2b3c4d-295832.nam'), {
    hash: '1a2b3c4d',
    size: 295832,
  });
  // A catalog URL is not a stash address.
  assert.equal(
    stashAddressOf('https://www.tone3000.com/api/v1/models/427332/download/x.nam'),
    null
  );
  assert.equal(stashAddressOf('file:///stash/plain-name.nam'), null);
});
