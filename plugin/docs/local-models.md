...
`metadata.modeled_by` (the author's username). What it has no room for is a
tone id, so the catalog entry has to be *found*. When it is, the tile shows
the real artwork, title, gear and author instead of the generic file glyph.
...
download, attempted once per session, so a miss, an outage or a signed-out
